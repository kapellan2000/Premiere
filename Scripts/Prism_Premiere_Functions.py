...
class Prism_Premiere_Functions(object):
...
#        self.callbacks.append(self.core.registerCallback("openPBListContextMenu", self.openPBListContextMenu))
    ...
    def startup(self, origin):
        ...
        if self.win:
            try:
                # CS6: .60, CC2015: .90
                pass
                #self.psApp = win32com.client.Dispatch("Premiere.Application")
            except Exception as e:
                QMessageBox.warning(
                    self.core.messageParent,
                    "Warning",
                    "Could not connect to Premiere:\n\n%s" % str(e),
                )
                return False
        else:
            self.psAppName = "Adobe Premiere CC 2019"
            for foldercont in os.walk("/Applications"):
                for folder in reversed(sorted(foldercont[1])):
                    if folder.startswith("Adobe Premiere"):
    ...
    def saveScene(self, origin, filepath, details={}):
        try:
                scpt ="app.project.saveAs('"+filepath+"');"

                name = self.executeAppleScript(scpt)
                if name is None:
                    raise
        except:
            self.core.popup("There is no active document in Premiere.")
            return False
    ...
    def onProjectBrowserStartup(self, origin):
        ...
        psMenu = QMenu("Premiere", origin)
        psAction = QAction("Open tools", origin)
        psAction.triggered.connect(self.openPremiereTools)
...
#        #return rcmenu
    ...
    def openScene(self, origin, filepath, force=False):
        # load scenefile
        if (not filepath.endswith(".prproj")):
            return False

        scpt ="app.openDocument('"+filepath+"');"
        name = self.executeAppleScript(scpt)
        return True
    ...
    def PremiereImportSource(self, origin, filepath=None):
        ...
        try:
            ...
            # curReadNode = Premiere.createNode("Read",'file %s first %s last %s' % (filePath,firstFrame,lastFrame),False)

    @err_catcher(name=__name__)
    def PremiereImportPasses(self, origin):
        ...
        for curPass in passes:
            ...
            curReadNode = Premiere.createNode(
                "Read",

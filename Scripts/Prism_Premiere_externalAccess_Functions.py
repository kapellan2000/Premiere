...
class Prism_Premiere_externalAccess_Functions(object):
    ...
    def projectBrowser_loadUI(self, origin):
        ...
        if self.core.appPlugin.pluginName == "Standalone":
            psMenu = QMenu("Premiere")
            psAction = QAction("Connect", origin)
            psAction.triggered.connect(lambda: self.connectToPremiere(origin))
...
#        return "55555"
    ...
    def customizeExecutable(self, origin, appPath, filepath):
        self.connectToPremiere(origin, filepath=filepath)
        return True

    @err_catcher(name=__name__)
    def connectToPremiere(self, origin, filepath=""):
